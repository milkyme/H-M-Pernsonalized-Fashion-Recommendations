...
- 2. Candidate generate
    ...
    - 물론 1) MAP@12는 12개만 추천하여 산출한 수치이고 candidate에는 수백개의 article 후보군이 있다. 그리고 2) MAP는 Mean Average Precision으로 어디까지나 기본적인 계산방식은 precision이라 recall과는 직접적인 비교가 불가능하긴 하다. 하지만 1) 12개와 수백개의 간극은 ranking 모델에서 한번 더 정교화된 추천을 진행할 것이므로  극복할 여지가 다분하다. 2) precision과 recall은 분명 다르긴 하지만, metric을 적용할 때 분모에 포함되는 요소는 달라도 분자에 포함되는 요소는 같다. 따라서 산출한 평균 recall의 수치도  최종적으로 사용해야 할 metric인 MAP와 충분한 상관관계가 존재한다.
