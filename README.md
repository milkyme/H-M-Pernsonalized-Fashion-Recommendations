...
- 3. Ranking model
    ...
    - ***하지만 valdation 데이터가 training을 하는 데에 사용되는 Data leakage가 일어나지 않도록 매우 유의하며 작업을 진행했고, validation 데이터를 최대한 test 데이터와 비슷하도록 설정해주었기에(같은 1주일 기간, 1주일밖에 차이나지 않는 시간대 등) 충분히 괜찮은 성능이 나왔다고 생각한다.***
